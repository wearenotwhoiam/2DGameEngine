...
namespace engine::ecs
{
    struct Scene
    {
        ...
        ENGINE_INLINE void Start()
        {
            ...
            //load texture
            auto texture = m_assets.LoadTexture("C:/GameDev/C++/ECS/engine/src/assets/tilemap.h", "tilemap", m_renderer);

            //create tilemap asset
            auto tilemap = m_assets.AddAsset<TilemapAsset>("tilemap");
            tilemap->instance.tilesets.insert(texture->id);
            tilemap->instance.colCount = 16;
            tilemap->instance.rowCount = 8;
